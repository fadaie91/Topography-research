...
simulation = Simulation(model, Δt = Δt, stop_time = 0.1, progress = progress, iteration_interval = 10)
...
function time_series_variance(prefix)
  ...
  ###### I know that the problem is in line 220 and 221
  ###### there should be a different way for openning and loading
  ###### Image files!
    ...
    iterations = parse.(Int, keys(file["timeseries/t"]))
     ...
     anim = @animate for (i, iter) in enumerate(iterations)
        ...
        tracer_final[i] = sum(interior(θ[i]))/(grid.Ny*grid.Nz)
        Ts[i] = tracer_final[i]

    end

  variance_plot = contourf(Ts[:]; title = "time_series_variance")
	savefig(variance_plot, "variance_plot.png")
	close(file)
end
time_series_variance("variance_plot")
...
tracer_initial_tot
tracer_final_tot
difference_error=(abs(tracer_initial_tot-tracer_final_tot)/tracer_initial_tot)*100
